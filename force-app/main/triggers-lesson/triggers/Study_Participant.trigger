...
trigger Study_Participant on Study_Participant__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    ...
    if(Trigger.isAfter && (Trigger.isInsert || Trigger.isUpdate)) StudyParticipantTriggerHandler.updateStudyParticipant((Map<Id, Study_Participant__c>)Trigger.oldMap, Trigger.new);
}
